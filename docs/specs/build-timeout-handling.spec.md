...
| F-10 | P0 | `cmd.WaitDelay` MUST be set to a non-zero grace period so that `Run()` cannot block past `limit + grace` when a grandchild inherits the output pipe. The grace period MUST be a constant in `internal/builder`, **5 seconds**, and MUST NOT be user-configurable. | Closes B-8, which is the difference between a documented bound and a real one. **Verified this session**: the same grandchild scenario returned at ≈10 s with no `WaitDelay` and at ≈800 ms with `WaitDelay = 500 ms`. |
